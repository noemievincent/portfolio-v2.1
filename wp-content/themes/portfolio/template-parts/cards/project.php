...
<article aria-labelledby="<?= sanitize_title($title) ?>"
    ...
    <div class="bg-theme-dark aspect-square w-full flex items-center justify-center">
        <?php if ($img_id = get_post_thumbnail_id()) : ?>
            <?= wp_get_attachment_image($img_id, 'large', false, ['class' => 'h-full w-full object-cover ease-all group-hover:scale-[1.15] group-focus:scale-[1.15]']) ?>
        <?php else : ?>
    ...
    <a href="<?= $url ?>"
       class="absolute z-1 inset-0 w-full h-full ease-all bg-white/70 flex flex-col p-4 opacity-0 group-hover:opacity-100 ease-all">
        <span class="text-center flex items-center justify-center aspect-square w-full font-light text-xl"><?= __('En savoir plus sur ce projet', 'nv_portfolio') ?></span>
